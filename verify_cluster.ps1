...
mongosh --port 27401 --eval "rs.status().members.map(m => m.name)"

Write-Host "`n[4] Paris Documents" -ForegroundColor Yellow
mongosh NosCites --port 27050 --eval "db.listing.countDocuments({data_city: 'Paris'})"

Write-Host "`n[5] Lyon Documents" -ForegroundColor Yellow
mongosh NosCites --port 27050 --eval "db.listing.countDocuments({data_city: 'Lyon'})"
